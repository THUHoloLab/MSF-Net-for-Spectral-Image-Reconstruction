...
def main():
    ...
    model = MSF_net_fusion(innput_channels=1, spectral_channels=29).cuda(CUDA_DEVICE)
    ...
    test_measurement = torch.cat(
        (test_measurement_CASSI, 0.25 * mask_CASSI.expand(test_measurement_CASSI.shape[0], -1, -1, -1)), dim=1).cuda(
        CUDA_DEVICE)
