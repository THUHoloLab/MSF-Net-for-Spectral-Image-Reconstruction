...
def train_model(args):
    ...
    test_measurement = torch.cat((test_measurement_gray, test_measurement_cassi,
                                  0.25 * mask_cassi.expand(test_measurement_gray.shape[0], -1, -1, -1)), dim=1).cuda(CUDA_DEVICE)
    ...
    for epoch in range(args.n_epoch):
        for train_hsi_batch in train_loader:
            ...
            measurement_gray = torch.sum(train_hsi_batch * mask_gray, dim=1, keepdim=True) / args.spectral_channel
            measurement_cassi = torch.sum(train_hsi_batch * mask_cassi, dim=1, keepdim=True) / args.spectral_channel
            optimizer.zero_grad()
            model_out = model(
                torch.cat([measurement_gray, measurement_cassi, 0.25 * mask_cassi.expand(train_hsi_batch.shape[0], -1, -1, -1)],
                          dim=1))
